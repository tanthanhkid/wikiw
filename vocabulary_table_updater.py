...
Cách sử dụng:
1. Chạy script để cập nhật tất cả các dòng trống
2. Script sẽ tự động lấy nội dung từ phần "## Khái Niệm" trong các file markdown
3. Nếu không có nội dung thực sự, sẽ thêm placeholder "Đang trong quá trình bổ sung..."
"""
...
def update_empty_cells_only():
    """
    Chỉ cập nhật các dòng có cột khái niệm trống
    """
    # Đọc file
    with open('content/TU-KHAINIEM/_index.md', 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Sử dụng regex để thay thế tất cả các dòng có cột khái niệm trống
    pattern = r'(\| \[([^\]]+)\]\(([^)]+)\) \|) \|'
    
    def replacement(match):
        full_match = match.group(1)
        return f"{full_match} Đang trong quá trình bổ sung... |"
    
    # Thay thế tất cả các match
    updated_content = re.sub(pattern, replacement, content)
    
    # Đếm số lượng thay thế
    matches = re.findall(pattern, content)
    print(f"Đã cập nhật {len(matches)} dòng trống")
    
    # Ghi lại file
    with open('content/TU-KHAINIEM/_index.md', 'w', encoding='utf-8') as f:
        f.write(updated_content)
    
    print("Hoàn thành cập nhật tất cả các dòng trống!")

if __name__ == "__main__":
    print("Chọn chế độ cập nhật:")
    print("1. Cập nhật với nội dung thực từ file markdown")
    print("2. Chỉ cập nhật các dòng trống với placeholder")
